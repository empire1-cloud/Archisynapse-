...
export function ArchitectureCanvas({ components, edges, selectedId, onSelect, onRemove, onDrag, onCanvasClick }: ArchitectureCanvasProps) {
  return (
    <div className="flex-1 relative overflow-hidden canvas-grid min-h-0" onClick={onCanvasClick}>
      <div className="relative w-full h-full bg-background overflow-hidden cursor-crosshair rounded-[3rem] border border-white/5">
          ...
          {components.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                ...
                <p className="text-muted-foreground text-sm mt-4">Search and add components to design your stack</p>
              </div>
            </div>
          )}
        </div>
